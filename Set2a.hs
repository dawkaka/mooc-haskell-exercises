...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs =  if diff < 0  then  xs else drop diff xs
...
palindromify s = if isPalindrome(s)
                 then s
                 else if isPalindrome(drop 1 s)
                 then drop 1 s
                 else if isPalindrome(take ((length s) - 1) s)
                 then take (length s) s
                 else palindromify ms
                 where ms = take (length s - 2) (drop 1 s)
...
safeIndex xs i = if i < 0 then Nothing 
...
addEithers (Right _) (Left v) = Left v
