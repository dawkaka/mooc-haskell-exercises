...
sums' i count prv = if count == i
                    then [newCount]
                    else newCount:(sums' i (count + 1) newCount)
                    where newCount = prv + count
...
sorted :: [Int] -> Bool
sorted xs =  sorted' xs (-1000)

sorted' :: [Int] -> Int -> Bool 
sorted' [] _ = True 
sorted' (x:xs) prv = if x < prv then False else sorted' xs x
...
sumsOf' :: [Int] -> Int -> [Int]
sumsOf' [] _ = []
sumsOf' (x:xs) prv = (x + prv ):(sumsOf' xs (x + prv))
...
merge x [] = x
merge [] [] = []
merge (x:xs) (y:ys) = if x < y then  x:(merge xs (y:ys)) else if y < x then y:merge (x:xs) ys else x:y:merge xs ys
